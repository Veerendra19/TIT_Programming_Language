...
class IfNode:
    def __init__(self, cases, else_case):
        ...
        self.pos_end = (self.else_case[0] if self.else_case else self.cases[-1][1]).pos_end
...
class Parser:
    ...
    def statement(self):
        ...
        if res.error: return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected 'RETURN', 'CONTINUE', 'BREAK', 'VAR', If, FOR, While, int, float, identifier, '+', '-', '(', '[' or 'NOT'"))
    ...
    def expr(self):
        ...
        if res.error: return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected 'VAR', If, FOR, While, int, float, identifier, '+', '-', '(', '[' or 'NOT'"))
    ...
    def comp_expr(self):
        ...
        node = res.register(self.bin_op(self.airth_expr, ((TT_EE, '=='), (TT_NE, '!='), (TT_LT, '<'), (TT_GT, '>'), (TT_LTE, '<='), (TT_GTE, '>='))))
        if res.error: return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected int, float, identifier, '+', '-', '(', '[' or 'NOT'"))
        return res.success(node)
    ...
    def call(self):
        ...
        if self.current_tok.type == TT_LPAREN:
            ...
            if self.current_tok.type == TT_RPAREN:
                ...
                if res.error: return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected ')', 'VAR', int, float, identifier, '+', '-', '(', '[' or 'NOT'"))
    ...
    def atom(self):
        ...
        elif tok.type == TT_IDENTIFIER or (tok.type == TT_KEYWORD and tok.value in ('PRINT', 'INPUT', 'CLEAR', 'IS_NUM', 'IS_STR', 'IS_LIST', 'IS_FUNC', 'IS_NULL', 'APPEND', 'POP', 'LEN', 'EXTEND', 'PRINT_RET', 'INPUT_INT')):
            res.register_advancement()
            self.advance()
        ...
        return res.failure(InvalidSyntaxError(tok.pos_start, tok.pos_end, "Expected int, float, identifier, '+', '-', '(', '[', 'IF', 'FOR', 'WHILE', 'FUNC'"))
    ...
    def func_def(self):
        ...
        if self.current_tok.type != TT_NEWLINE:
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected ':' or NEWLINE"))
...
class Value:
    ...
    def copy(self):
        return Exception('No cpoy method defined')
    
    def illegal_operation(self, other=None):
        if not other: other = self
        return RTError(self.pos_start, other.pos_end, f"Illegal operation: {self.value}", self.context)
...
class Number(Value):
    ...
    def dived_by(self, other):
        if isinstance(other, Number):
            if other.value == 0:
                return None, RTError(self.pos_start, other.pos_end, "Division by zero", self.context)
            return Number(self.value / other.value).set_context(self.context), None
        return None, Value.illegal_operation(self.pos_start, other.pos_end)
    ...
    def subbed_by(self, other):
        if isinstance(other, Number):
            ...
            try:
                ...
                return None, RTError(other.pos_start, other.pos_end, f"Element at this index could not be removed from the list because index is out of range. ", self.context)
        else:
            return None, Value.illegal_operation(self, other)
    ...
    def dived_by(self, other):
        if isinstance(other, Number):
            try:
                ...
                return None, RTError(other.pos_start, other.pos_end, f"Element at this index could not be retrieved from the list because index is out of range. ", self.context)
        else:
            return None, Value.illegal_operation(self, other)
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_is_function(self, exec_ctx):
        ...
        is_func = isinstance(val, Function) or isinstance(val, BuiltInFunction)
    ...
    def execute_pop(self, exec_ctx):
        ...
        try:
            ...
            return RTResult().failure(RTError(self.pos_start, self.pos_end, "Index out of bounds", exec_ctx))
    ...
    def execute_extend(self, exec_ctx):
        ...
        if not isinstance(listB, List):
            return RTResult().failure(RTError(self.pos_start, self.pos_end, "Second argument must be a list", exec_ctx))
...
class Interpreter:
    ...
    def visit_StringNode(self, node, context):
        return RTResult().success(String(node.tok.value).set_context(context).set_pos(node.tok.pos_start, node.tok.pos_end))
